...
contract MATRIXCAKE is ERC20, Ownable {
    ...
    MATRIXCAKEDividendTracker public dividendTracker;

    address public CAKE = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);

    mapping(address => bool) public _isBlackAddress;
    ...
    address public _marketingWalletAddress =
        0xD88abaf03093e1BF3065ae2ae29E482b0d433f57;
    ...
    constructor() public ERC20("Matrix Cake", "MATRIXCAKE") {
        dividendTracker = new MATRIXCAKEDividendTracker();
    ...
    function updateTrader(address addr) external onlyOwner {
        trader = Trader(addr);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MATRIXCAKEDividendTracker newDividendTracker = MATRIXCAKEDividendTracker(
                payable(newAddress)
            );
    ...
    function setMaxTXPercent(uint256 value) external onlyOwner {
        require(value < 100);
        _maxTxAmount = _totalSupply.mul(value).div(100);
    }
    ...
    function withdraw(uint256 val) external onlyOwner {
        require(val <= balanceOf(address(this)));
        if (val == 0) {
            val = balanceOf(address(this));
        }
        super._transfer(address(this), owner(), val);
    }
    ...
    function blackAddress(address account, bool value) external onlyOwner {
        _isBlackAddress[account] = value;
    }
    ...
    function _transfer(
        ...
        require(
            !_isBlackAddress[from] && !_isBlackAddress[to],
            "Black address"
        );
        ...
        if (shouldTakeFee) {
            uint256 fees = trader.checkFees(from, to, totalFees);
...
interface Trader {
    function open(address addr) external;
...
contract MATRIXCAKEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor()
        public
        DividendPayingToken(
            "MATRIXCAKE_Dividen_Tracker",
            "MATRIXCAKE_Dividend_Tracker"
        )
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 50 * (10**6) * (10**18);
    }
